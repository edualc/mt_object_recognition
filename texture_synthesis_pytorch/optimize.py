...
class Optimizer:
    def __init__(self, model: model.Model, args: Namespace, start_image: None):
        ...
        if start_image is None:
            desired_noise = torch.randn_like(model.target_image)
        else:
            desired_noise = start_image

            # In same or larger ranges are used for start_image
            if (model.target_image.max() <= start_image.max()) or (model.target_image.min() >= start_image.min()):
                desired_noise = torch.div(desired_noise, 2)

        # make sure the inverse is defined for these values
        desired_noise = desired_noise.clamp(
            float(model.target_image.min()),
            float(model.target_image.max())
        )
